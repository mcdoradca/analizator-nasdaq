# -*- coding: utf-8 -*-
"""Naprawiony Agent Złotej Ligi

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1_d_zSP8RCyi1X2YA-u46DrmU_Dhvlgif
"""
...
# NAPRAWIONO: Poprawiono import z relatywnego na bezpośredni, aby zapewnić kompatybilność z Render.
...
def expert_agent_fundamentalista(overview_data):
    ...
    score = 50
    try:
        if not overview_data: return 50
...
def expert_agent_kwant(daily_data, overview_data):
    ...
    score = 50
    try:
        if not daily_data or not overview_data: return 50
...
def expert_agent_straznik(news_data, ticker):
    ...
    Zwraca wynik w skali 0-100.
    """
    score = 60 # Neutralny punkt wyjściowy, jeśli brak wiadomości
    try:
        if not news_data or 'feed' not in news_data or not news_data['feed']:
            return score

        sentiments = [
            safe_float(s.get('sentiment_score', 0)) for item in news_data['feed']
...
def run_zlota_liga_analysis(tickers, data_fetcher):
    ...
    for ticker in tickers:
        ...
        try:
            ...
            # Walidacja kluczowych danych przed kontynuacją
            if not all([overview, daily, quote]):
                print(f"[Złota Liga] Pomijam {ticker} z powodu braku kluczowych danych (overview/daily/quote).")
                continue
